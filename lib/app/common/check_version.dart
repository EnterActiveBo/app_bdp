...
class AppVersionChecker {
  ...
  Future<AppCheckerResult> checkUpdate() async {
    ...
    final currentVersionResponse = currentVersion ?? packageInfo.version;
    final packageName = appId ?? packageInfo.packageName;
    if (Platform.isAndroid) {
      switch (androidStore) {
        case AndroidStore.apkPure:
          return await _checkApkPureStore(currentVersionResponse, packageName);
        default:
          return await _checkPlayStore(currentVersionResponse, packageName);
      }
    } else if (Platform.isIOS) {
      return await _checkAppleStore(currentVersionResponse, packageName);
    } else {
      return AppCheckerResult(currentVersionResponse, null, "",
